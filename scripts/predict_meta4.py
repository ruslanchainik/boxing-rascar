"""Meta-ensemble with 4 feature groups: orig (426), eng (464), glove (492), cnn-cloud (860=pose+cnn368)."""
...
def parse_args():
    ...
    p.add_argument("--ckpts_cnn", nargs="+", required=True)
    ...
    p.add_argument("--features_cnn", type=Path, default=Path("artifacts/cnn_features"))
    p.add_argument("--w_orig", type=float, default=1.0)
    p.add_argument("--w_eng", type=float, default=1.0)
    p.add_argument("--w_glove", type=float, default=1.0)
    p.add_argument("--w_cnn", type=float, default=1.0)
...
def main():
    ...
    models_c = load_models(args.ckpts_cnn, device)
    print(f"[meta4] orig={len(models_o)} eng={len(models_e)} glove={len(models_g)} cnn={len(models_c)} tta={args.tta}")
    ...
    for _, vrow in test_videos.iterrows():
        ...
        pc = args.features_cnn / f"{vk}.npz"
        if not (po.exists() and pe.exists() and pg.exists() and pc.exists()):
        ...
        feats_cnn = np.load(pc)["features"].astype(np.float32)
        fps_native = float(np.load(po)["fps"]) if "fps" in np.load(po).files else 30.0
        T = min(len(feats_o), len(feats_e), len(feats_g), len(feats_cnn))
        feats_o = feats_o[:T]; feats_e = feats_e[:T]; feats_g = feats_g[:T]
        feats_cnn_combo = np.concatenate([feats_g[:T], feats_cnn[:T]], axis=1)  # 492 + 368 = 860
        ...
        # CNN-cloud model doesn't use color_swap (different feature layout for cnn part)
        ev_c, at_c, _ = forward_group(models_c, feats_cnn_combo, device, tta=False)

        wo, we, wg, wc = args.w_orig, args.w_eng, args.w_glove, args.w_cnn
        tw = wo + we + wg + wc
        ev = (wo*ev_o + we*ev_e + wg*ev_g + wc*ev_c) / tw
        attrs = {n: (wo*at_o[n] + we*at_e[n] + wg*at_g[n] + wc*at_c[n]) / tw for n in ATTR_NAMES}

        peaks = peak_pick(ev, args.threshold, args.min_distance)[:200]
        print(f"[meta4] {vk}: {len(peaks)} peaks")
    ...
    print(f"[meta4] wrote {args.out} rows={len(pred_df)} clear_true={n_real}")
